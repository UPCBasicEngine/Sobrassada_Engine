...
namespace ImGui
{
    template <int steps> void bezier_table(ImVec2 P[4], ImVec2 results[steps + 1])
    {
    ...
    int Bezier(const char* label, float P[5])
    {
        ...
        // header and spacing
        int changed  = SliderFloat4(label, P, 0, 1, "%.3f", ImGuiSliderFlags_None);
        bool hovered = IsItemActive() || IsItemHovered(); // IsItemDragged() ?
        Dummy(ImVec2(0, 3));
        ...
        hovered          |= ImGui::ItemHoverable(ImRect(bb.Min, bb.Min + ImVec2(avail, dim)), id, 0);
        ...
        {
            DrawList->AddLine(
                ImVec2(bb.Min.x + i, bb.Min.y), ImVec2(bb.Min.x + i, bb.Max.y), GetColorU32(ImGuiCol_TextDisabled)
            );
        }
        for (int i = 0; i <= Canvas.y; i += ((int)Canvas.y / 4))
        {
            DrawList->AddLine(
                ImVec2(bb.Min.x, bb.Min.y + i), ImVec2(bb.Max.x, bb.Min.y + i), GetColorU32(ImGuiCol_TextDisabled)
            );
        }
