...
namespace ImGui
{
    ...
    int Curve(
        const char* label, const ImVec2& size, const int maxpoints, ImVec2* points, int* selection,
    ...
    )
    {
        ...
        if (hovered)
        {
            ...
            else if (g.IO.MouseDoubleClicked[0]) action = action_delete_point;
        ...
        {
            drawList->AddLine(
                ImVec2(bb.Min.x + (wd / 10) * (i + 1), bb.Min.y), ImVec2(bb.Min.x + (wd / 10) * (i + 1), bb.Max.y),
                gridColor2
            );
        }
        ...
        if (hovered || draggingPoint)
        {
            ...
            for (i = 0; i < pointCount; i++)
            {
                ...
                if (hoveredPoint == i) drawList->AddRect(a, b, GetColorU32(ImGuiCol_PlotLinesHovered));
        ...
        RenderTextClipped(
            ImVec2(bb.Min.x, bb.Min.y + style.FramePadding.y), bb.Max, str, NULL, NULL, ImVec2(0.5f, 0.5f)
        );
        ...
        static const char* items[] = {"Custom",
                                      ...
                                      "Cubic in",        "Cubic out",     "Cubic in  out", "Quart in",
                                      "Quart out",       "Quart in  out", "Quint in",      "Quint out",
                                      "Quint in  out",   "Sine in",       "Sine out",      "Sine in  out",
                                      "Expo in",         "Expo out",      "Expo in  out",  "Circ in",
                                      "Circ out",        "Circ in  out",  "Elastic in",    "Elastic out",
                                      "Elastic in  out", "Back in",       "Back out",      "Back in  out",
