...
void ResourceMesh::LoadVBO(
    const tinygltf::Model& inModel, const tinygltf::Mesh& inMesh, const tinygltf::Primitive& inPrimitive
)
{
    /*	// Getting required size for VBO buffer
            ...
            if (tangentIterator != inPrimitive.attributes.end())
        {
            ...
            {
                    unsigned int bufferSize = (sizeof(float) * 3 * vertexCount) + (sizeof(float) * 2 *
       textureCoordCount) + (sizeof(float) * 3 * normalCoordCount) + (sizeof(float) * 4 * tangentCoordCount);
                    glGenBuffers(1, &vbo);
                    glBindBuffer(GL_ARRAY_BUFFER, vbo);
            ...
            if (vertexCount > 0)
            {
                    ...
                    //maximumPosition = float3((float)positionAccessor.maxValues[0],
       (float)positionAccessor.maxValues[1], (float)positionAccessor.maxValues[2]);
                    //minimumPosition = float3((float)positionAccessor.minValues[0],
       (float)positionAccessor.minValues[1], (float)positionAccessor.minValues[2]); const float3 maximumPosition =
       float3((float)positionAccessor.maxValues[0], (float)positionAccessor.maxValues[1],
       (float)positionAccessor.maxValues[2]); const float3 minimumPosition =
       float3((float)positionAccessor.minValues[0], (float)positionAccessor.minValues[1],
       (float)positionAccessor.minValues[2]); aabb = AABB(minimumPosition, maximumPosition);
                    ...
                    const unsigned char* bufferStart = &(positionBuffer.data[positionAccessor.byteOffset +
       positionBufferView.byteOffset]);

                    float3* ptr = reinterpret_cast<float3*>(glMapBufferRange(GL_ARRAY_BUFFER, 0, sizeof(float) * 3 *
       vertexCount, GL_MAP_WRITE_BIT));
       ...
       space between vertices -> sizeof(float) * 3. bufferStart += positionBufferView.byteStride == 0 ? sizeof(float) *
       3 : positionBufferView.byteStride;
                    }
            ...
            if (textureCoordCount > 0)
            {
                    ...
                    const unsigned char* bufferStart = &(textureBuffer.data[textureAccessor.byteOffset +
       textureBufferView.byteOffset]);

                    float2* ptr = reinterpret_cast<float2*>(glMapBufferRange(GL_ARRAY_BUFFER, sizeof(float) * 3 *
       vertexCount, sizeof(float) * 2 * textureCoordCount, GL_MAP_WRITE_BIT));
                    ...
                    for (size_t i = 0; i < textureAccessor.count; ++i)
                    {
                            ...
                            // bufferView.byteStride == 0 -> Only positions inside buffer, which then the stride becomes
       space between vertices -> sizeof(float) * 3. bufferStart += textureBufferView.byteStride == 0 ? sizeof(float) * 2
...
)
{
    /*if (inPrimitive.indices >= 0)
    {
            ...
            const unsigned char* bufferStart = &(indexBuffer.data[indexAccessor.byteOffset +
...
void ResourceMesh::CreateVAO()
{
    ...
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 0, (void*)((sizeof(float) * 3 * vertexCount) + (sizeof(float) * 2 *
textureCoordCount)));
    }
else
{
