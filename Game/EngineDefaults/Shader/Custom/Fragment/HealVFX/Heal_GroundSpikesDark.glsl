...
void main()
{
    ...
    const vec2 addNode = invertedUvs + vec2(0.0f, 1.1f - (frameTimer - 1.2f) * 0.15f);
    ...
    const float scalar = clamp((frameTimer - 1.2f) * 0.03f - 0.2f, 0.0f, 1.0f);
    const vec4 finalAlpha = textureNode * scalar * 3.0f;
